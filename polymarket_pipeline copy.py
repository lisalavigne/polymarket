...
def select_relevant_event_universe(
    ...
    *,
    ...
    events_df_selected = events_df_full[
        events_df_full["slug"].isin(filtered_events_legacy_df["slug"])
    ...
    markets_filtered = markets_df[
        markets_df["event_slug"].isin(events_df_selected["slug"])
    ].copy()
    ...
    prices_long_df_filtered = prices_long_df[
        prices_long_df["market_id"].isin(markets_filtered["id"])
    ].copy()
...
def export_pipeline_outputs(results: dict, output_dir: Path) -> None:
    ...
    for frame, filename, sheet_name, columns in zip(excel_frames, output_filenames, sheet_names, wrap_columns):
        ...
        print(f"[export] wrote {filename} rows={len(frame)}")

    if "daily_features_df" in results and not results["daily_features_df"].empty:
        daily_features_export = results["daily_features_df"].copy()
        dt = pd.to_datetime(daily_features_export["ts"], errors="raise")
        daily_features_export["ts"] = dt.dt.tz_convert("US/Eastern").dt.tz_localize(None)
    ...
    for key, filename in [
        ...
        if isinstance(frame, pd.DataFrame):
            frame.to_excel(output_dir / filename, index=False)
            print(f"[export] wrote {filename} rows={len(frame)}")
...
def run_pipeline(
    *,
    ...
    print(f"[pipeline] start tags={len(tags_df)} run_ai={run_ai} export={export}")
    ...
    selected_universe = select_relevant_event_universe(
        events_df_full,
        markets_df,
        prices_long_df,
        theme_name=legacy_theme_name,
        theme_overrides=legacy_theme_overrides,
    )
    ...
    if export:
        output_dir = output_dir or Path("/domino/edv/GS_CUSOERM_LVOVERRIDE_RW")
        export_pipeline_outputs(results, output_dir)
...
def main() -> None:
    ...
    run_pipeline(export=bool(output_dir), output_dir=output_dir)
